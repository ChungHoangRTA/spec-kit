...
case "$AGENT_TYPE" in
    ...
    "gemini") 
        ...
        ;;
    "qwen")
        update_agent_file "$QWEN_FILE" "Qwen CLI"
        ;;
    "copilot")
    ...
    "")
        ...
        [ -f "$QWEN_FILE" ] && update_agent_file "$QWEN_FILE" "Qwen CLI"
        [ -f "$COPILOT_FILE" ] && update_agent_file "$COPILOT_FILE" "GitHub Copilot"
        
        # If no files exist, create based on current directory or ask user
        if [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$QWEN_FILE" ] && [ ! -f "$COPILOT_FILE" ]; then
            echo "No agent context files found. Creating Claude Code context file by default."
            update_agent_file "$CLAUDE_FILE" "Claude Code"
        fi
        ;;
    *)
        echo "ERROR: Unknown agent type '$AGENT_TYPE'. Use: claude, gemini, qwen, copilot, or leave empty for all."
        exit 1
        ;;
esac
echo ""
...
echo ""
echo "Usage: $0 [claude|gemini|qwen|copilot]"
